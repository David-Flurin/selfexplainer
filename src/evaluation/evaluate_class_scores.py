...
for batch in tqdm(data_module.test_dataloader()):
        ...
        for target_class in intersection:
            ...
            for method in methods:
                if method == 'selfexplainer':
                    continue
                all_scores[method][target_class]['None'].append(target_prob)
    ...
    if 'selfexplainer' in methods:
        ...
        if intersection:
            for target_class in intersection:
                target_prob = output_probs[target_class].cpu().numpy()
                all_scores['selfexplainer'][target_class]['None'].append(target_prob)
        ...
        for method in methods:
            for target_class in intersection:
                ...
                for mask_class in mask_classes:
                        ...
                        mask = np.load(masks_dir / filename)['arr_0']
