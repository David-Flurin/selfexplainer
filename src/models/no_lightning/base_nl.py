...
class BaseModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.writer.add_scalar('classification_loss', classification_loss, global_step=self.i)   

        if self.use_similarity_loss:
            self.writer.add_scalar('classification_loss_1Pass', classification_loss, global_step=self.i)   
            self.writer.add_scalar('classification_loss_2Pass', self.classification_loss_fn(output['object'][3], target_vector), global_step=self.i)     
        ...
        if self.use_similarity_loss:
            ...
            self.writer.add_scalar('similarity_loss', similarity_loss, global_step=self.i)
        ...
        if self.use_background_loss:
            ...
            self.writer.add_scalar('background_loss', background_entropy_loss, global_step=self.i)
        ...
        self.writer.add_scalar('TV mask loss', mask_variation_loss, global_step=self.i)
        ...
        self.writer.add_scalar('Bounding area loss', bounding_area_loss, global_step=self.i)
        ...
        self.writer.add_scalar('Mean area loss', mask_area_loss, global_step=self.i)
        ...
        if self.use_similarity_loss or self.use_background_loss:
            if self.use_weighted_loss:
                ...
                self.writer.add_scalar('weighted_loss', w_obj_back_loss, global_step=self.i)
                w_mask_loss = weighted_loss(loss, mask_loss, 5, 0.1)
                self.writer.add_scalar('Weighted mask losses', w_mask_loss, global_step=self.i)
                loss += w_obj_back_loss + w_mask_loss
                #w_m_loss = weighted_loss(loss, mask_loss, 10, 0.1)
                #self.writer.add_scalar('weighted mask loss', w_m_loss, global_step=self.i)
                #loss += w_m_loss
            else:
        ...
        if self.use_background_activation_loss:
            ...
            self.writer.add_scalar('bg_logits_loss', bg_logits_loss, global_step=self.i)
        ...
        if self.i % 5 == 4:
            ...
            self.writer.add_image('Train Masked Images', get_unnormalized_image(masked_image), self.i, dataformats='NCHW')
            self.writer.add_image('Train Images', get_unnormalized_image(image), self.i, dataformats='NCHW')
            self.writer.add_image('Train 1PassOutput', output['image'][1].unsqueeze(1), self.i, dataformats='NCHW')
            if self.use_similarity_loss:
                self.writer.add_image('Train 2PassOutput', output['object'][1].unsqueeze(1), self.i, dataformats='NCHW')
            log_string = ''
            ...
            log_string += '  \n'
            self.writer.add_text('Train Logits', log_string,  self.i)

        self.writer.add_scalar('loss', float(loss), global_step=self.i)
        ...
        # o.step()
        self.i += 1.
        self.writer.add_scalar('iterations', self.i, global_step=self.i)
        return loss

    def training_epoch_end(self):
        m = self.train_metrics.compute()
        self.writer.add_scalars('train_metric', m, global_step=self.i)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        classification_loss = classification_loss_initial
        self.writer.add_scalar('val_classification_loss', classification_loss, global_step=self.i)        
        ...
        if self.use_similarity_loss:
            ...
            self.writer.add_scalar('val_similarity_loss', similarity_loss, global_step=self.i)
        ...
        if self.use_background_loss:
            ...
            self.writer.add_scalar('val_background_loss', background_entropy_loss, global_step=self.i)
            obj_back_loss += background_entropy_loss # Entropy loss is negative, so is added to loss here but actually its subtracted
        ...
        if self.use_mask_variation_loss:
            ...
            mask_area_loss = self.mask_total_area_regularizer * (output['image'][1].mean()) #+ output['object'][1].mean())
            #mask_area_loss += self.ncmask_total_area_regularizer * (output['image'][2].mean()) #+ output['object'][2].mean())
            self.writer.add_scalar('mask_area_loss', mask_area_loss, global_step=self.i)
            if self.use_mask_area_loss:
                mask_loss += mask_area_loss
        ...
        if self.use_similarity_loss or self.use_background_loss or self.use_mask_variation_loss or self.use_mask_area_loss:
            if self.use_weighted_loss:
                ...
                self.writer.add_scalar('Weighted loss', w_loss, global_step=self.i)
                loss += w_loss
            else:
                loss = loss + obj_back_loss + mask_loss
        ...
        if self.use_background_activation_loss:
            bg_logits_loss = self.bg_activation_regularizer * background_activation_loss(output['image'][1])
            self.writer.add_scalar('val_bg_logits_loss', bg_logits_loss, global_step=self.i)
            loss = weighted_loss(loss, bg_logits_loss, 2, 0.1)
        

        masked_image = output['image'][1].unsqueeze(1) * image
        self.logger.experiment.add_image('Val Masked Images', get_unnormalized_image(masked_image), self.i, dataformats='NCHW')
        self.writer.add_scalar('val_loss', float(loss), global_step=self.i)
    ...
    def validation_epoch_end(self, outs):
        ...
        self.writer.add_scalar('valid_metric', m, global_step=self.i)
        for k,v in m.items():
            self.writer.add_scalar(f'{k}', v, prog_bar=True, logger=False, global_step=self.i)
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.writer.add_scalar('test_loss', loss, global_step=self.i)
    ...
    def test_epoch_end(self, outs):
        ...
        self.writer.add_scalar('test_metric', m, global_step=self.i)
        for k,v in m.items():
            self.writer.add_scalar(f'{k}', v, prog_bar=True, logger=False, global_step=self.i)
