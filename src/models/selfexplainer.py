...
from utils.helper import get_class_dictionary, get_filename_from_annotations, get_targets_from_annotations, extract_masks, get_targets_from_segmentations, get_class_weights
from utils.image_display import save_all_class_masked_images, save_mask, save_masked_image, save_image, save_all_class_masks, get_unnormalized_image
from utils.loss import TotalVariationConv, ClassMaskAreaLoss, entropy_loss, weighted_loss, bg_loss, similarity_loss_fn
...
class SelfExplainer(pl.LightningModule):
    def __init__(self, num_classes=20, dataset="VOC", learning_rate=1e-4, pretrained=False, weighting_koeff=1., use_similarity_loss=True, similarity_regularizer=0.5, use_background_loss=True, bg_loss_regularizer=1.0, use_weighted_loss=True,
                use_mask_area_loss=True, use_mask_variation_loss=False, mask_variation_regularizer=1.0, ncmask_total_area_regularizer=0.3, mask_area_constraint_regularizer=1.0, class_mask_min_area=0.05, class_mask_max_area=0.3, 
                mask_total_area_regularizer=0.1, save_masked_images=False, save_masks=False, save_all_class_masks=False, background_loss='logits_ce', aux_classifier=False, multilabel=False, use_bounding_loss=True, 
                ...
                frozen=True, use_mask_logit_loss=False, mask_logit_loss_regularizer=1.0, mask_loss_weighting_params=[5, 0.1], object_loss_weighting_params=[2, 0.2]):

        super().__init__()

        self.model = Deeplabv3Resnet50Model(num_classes=num_classes, pretrained=pretrained, aux_classifier=aux_classifier)
        self.gpu = gpu
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.use_similarity_loss:
            ...
            self.log('test_similarity_loss', sim_loss.item())
            obj_back_loss += sim_loss.squeeze()
        ...
        if self.use_background_loss:
            ...
            self.log('test_background_loss', background_entropy_loss.item())
            obj_back_loss += background_entropy_loss.squeeze() # Entropy loss is negative, so is added to loss here but actually its subtracted
        ...
        bounding_area_loss = self.mask_area_constraint_regularizer * (self.class_mask_area_loss_fn(output['image'][0] if 'image' in output else output[0], target_vector)) #+ self.class_mask_area_loss_fn(output['object'][0], target_vectori))
        self.log('test_bounding_area_loss', bounding_area_loss.item())
        if self.use_bounding_loss:
            mask_loss += bounding_area_loss

        mask_area_loss = self.mask_total_area_regularizer * ((output['image'][1] if 'image' in output else output[1]).mean()) 
        mask_area_loss += self.ncmask_total_area_regularizer * ((output['image'][2] if 'image' in output else output[2]).mean())
        self.log('test_mean_area_loss', mask_area_loss.item())
        if self.use_mask_area_loss:
            mask_loss += mask_area_loss
        ...
        if self.use_mask_logit_loss:
            mask_logit_loss = self.mask_logit_loss_regularizer * self.classification_loss_fn(output['image'][4] if 'image' in output else output[4], target_vector)
            self.log('test_mask_logit_loss', mask_logit_loss.item())
            loss += mask_logit_loss
